...
class ConvGRUCell(nn.Module):
    ...
    def forward(self, input_cur, state_prev):
        ...
        gates_s = self.gate_conv(input_concat) # (b, 2, h, w)
        gate_update_s, gate_reset_s = gates_s.chunk(2, 1) # (b, 1, h, w)
        ...
        gate_update = torch.sigmoid(gate_update_s * gate_update_c.unsqueeze(-1).unsqueeze(-1))
        gate_reset = torch.sigmoid(gate_reset_s * gate_reset_c.unsqueeze(-1).unsqueeze(-1))
