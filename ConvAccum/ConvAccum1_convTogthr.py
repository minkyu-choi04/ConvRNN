...
import Gaussian2d_mask_generator_v1_tensorSigma as G

class ConvAccumCell(nn.Module):
    def __init__(self, input_c, hidden_c, input_s=(14,14), sigma=25.0):#, device='cuda'):
        super().__init__()
        self.input_s = input_s
        self.conv_out = nn.Conv2d(hidden_c, hidden_c, kernel_size=3, stride=1, padding=1, padding_mode='replicate')
        self.conv_in = nn.Conv2d(input_c+hidden_c, hidden_c, kernel_size=3, stride=1, padding=1, padding_mode='replicate')
        self.norm_out = nn.GroupNorm(32, hidden_c)
        self.norm_in = nn.GroupNorm(32, hidden_c)
        self.relu = nn.ReLU()
        self.sigma = nn.Parameter(torch.tensor([sigma])#, device='cuda'))

        self.init_hidden = nn.Parameter(torch.abs(torch.randn((1, hidden_c, *input_s))#, device=device)))
    ...
    def forward(self, fixs, inputs, state_prev=None):
        ...
        inputs_cat = torch.cat((inputs, state_prev), 1)
        state_in = self.relu(self.norm_in(self.conv_in(inputs_cat)))
